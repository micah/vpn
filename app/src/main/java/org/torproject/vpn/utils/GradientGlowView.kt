...
class GradientGlowView @JvmOverloads constructor(
    ...
    fun setState(connectionState: ConnectionState, hasConnectivity: Boolean) {
        colorConfig = when (connectionState) {
            ...
            ConnectionState.CONNECTED -> if (hasConnectivity) StaticColorConfig(R.color.tertiary, previousConfig = colorConfig) else StaticColorConfig(R.color.error, previousConfig = colorConfig)
